...
describe('eWeLink Set Status tests', () => {
	...
	it('Should be loaded', done => {
		const flow = [
	      ...
	      { id: 'n2', type: 'ewelink-set-status', auth: 'n1', name: 'Devices Node 123' }
	    ];
	    helper.load([authNode, setStatusNode], flow, () => {
	...
	it('Should send error -2 if no connection client retreived', done => {
		const flow = [
			{id: 'n1', type: 'ewelink-auth'},
			{id: 'n2', type: 'ewelink-set-status', auth: 'n1', wires:[['n3']]},
    		{ id: 'n3', type: 'helper' }
		];
	...
	it('Should send the error occured during the call to ewelink', done => {
		// override the device.setThingStatus method of the client
		const device = { setThingStatus: () => { }};
		const deviceStub = sinon.stub(device, 'setThingStatus').callsFake(() => Promise.reject('My Nasty Error'));
		...
		const flow = [
			...
			{id: 'n3', type: 'ewelink-set-status', auth: 'n1', wires:[['n4']]},
    		{id: 'n4', type: 'helper' }
		];
	...
	it('Should send back eWeLink errors in the output', done => {
		// override the device.setThingStatus method of the client
		const device = { setThingStatus: () => { }};
		const deviceStub = sinon.stub(device, 'setThingStatus').callsFake(() => Promise.resolve({
			error: 1,
			data: null,
