...
const getStatusNode = require("../src/ewelink-status-device/ewelink-status-device.js");
const authNode = require("../src/ewelink-auth/ewelink-auth.js");
const util = require("../src/ewelink-util/ewelink-util.js");
const sinon = require('sinon');

describe('eWeLink Status Device tests', () => {
	afterEach(() => {
    	helper.unload();
	});
	...
	it('Should be loaded', done => {
		const flow = [
	      ...
	      { id: 'n2', type: 'ewelink-status-device', auth: 'n1', name: 'Devices Node 123' }
	    ];
	    helper.load([authNode, getStatusNode], flow, () => {
	...
	it('Should send error -2 if no connection client retreived', done => {
		const flow = [
			{id: 'n1', type: 'ewelink-auth'},
			{id: 'n2', type: 'ewelink-status-device', auth: 'n1', wires:[['n3']]},
    		{ id: 'n3', type: 'helper' }
		];
	...
	it('Should send the error occured during the call to ewelink', done => {
		// override the device.getThingStatus method of the client
		const device = { getThingStatus: () => { }};
		const deviceStub = sinon.stub(device, 'getThingStatus').callsFake(() => Promise.reject('My Nasty Error'));
		...
		const flow = [
			...
			{id: 'n3', type: 'ewelink-status-device', auth: 'n1', wires:[['n4']]},
    		{id: 'n4', type: 'helper' }
		];
	...
	it('Should send back eWeLink errors in the output', done => {
		// override the device.getThingStatus method of the client
		const device = { getThingStatus: () => { }};
		const deviceStub = sinon.stub(device, 'getThingStatus').callsFake(() => Promise.resolve({
			error: 1,
			data: null,
